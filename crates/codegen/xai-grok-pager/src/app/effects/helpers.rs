...
pub(crate) async fn persist_setting(
    ...
    match key {
        ...
        "custom_model_api_backend" => {
            let SettingValue::Enum(s) = value else {
                return Err(kind_mismatch("custom_model_api_backend", "Enum", &value));
            };
            xai_grok_shell::util::config::set_custom_model_api_backend(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "custom_model_fetch_models" => {
            let SettingValue::Bool(b) = value else {
                return Err(kind_mismatch("custom_model_fetch_models", "Bool", &value));
            };
            xai_grok_shell::util::config::set_custom_model_fetch_models(b)
                .await
                .map_err(|e| e.to_string())
        }
        "custom_model_selected" => {
            let SettingValue::String(s) = value else {
                return Err(kind_mismatch("custom_model_selected", "String", &value));
            };
            xai_grok_shell::util::config::set_custom_model_selected(s)
                .await
                .map_err(|e| e.to_string())
        }
